...
sink connector is configured with the same `snowflake.schema.name` /
`db.schema` value, sinks that build a 3-tier URN can produce a doubled
schema segment — e.g. `mydb.public.public.users`.

These tests cover three areas:

1. (Postgres source) The connector configuration's `database.dbname` itself
   contains a dot (`mydb.public`). Without a guard, `get_dataset_name`
   concatenates `mydb.public` + `.` + `public.users` =
   `mydb.public.public.users`. The fix in `get_dataset_name` keeps only the
   first segment of `database_name` and warns.

2. (JDBC sink, e.g. Postgres / SQL Server / Oracle) Topic name already
...
3. (Snowflake sink, pinned-current-behavior) Two cases pinned: with and
...
class _MockSchemaResolver(SchemaResolverInterface):
    """Returns the URNs supplied at construction; everything else is a no-op."""
    ...
    def resolve_table(self, table: Any) -> Tuple[str, Optional[Dict[str, str]]]:
        urn = (
            f"urn:li:dataset:(urn:li:dataPlatform:{self._platform},{table.table},PROD)"
        )
        return urn, None
...
def _make_postgres_cdc_v2_connector(
    extra_config: Dict[str, str],
    schema_resolver: Optional[_MockSchemaResolver] = None,
) -> DebeziumSourceConnector:
    """Build a DebeziumSourceConnector wrapping a PostgresCdcSourceV2 manifest."""
    config_dict = {
...
def test_dotted_database_dbname_does_not_double_schema() -> None:
    """A misconfigured `database.dbname` containing a dot must not produce a
    duplicated schema segment in the lineage source URN.

    Some Confluent Cloud setups put a schema-qualified value into
    `database.dbname` (e.g. "mydb.public"). Without a guard,
    `get_dataset_name` produces `mydb.public.public.users`. The fix in
    `get_dataset_name` keeps only the first segment of `database_name` and
    logs a warning, so the URN becomes `mydb.public.users`.
    """
    resolver = _MockSchemaResolver(
        platform="postgres",
        mock_urns=[
            "urn:li:dataset:(urn:li:dataPlatform:postgres,mydb.public.users,PROD)"
        ],
    )
    connector = _make_postgres_cdc_v2_connector(
        extra_config={
            "database.dbname": "mydb.public",
            "table.include.list": "public.users",
        },
        schema_resolver=resolver,
    )

    parser = connector.get_parser(connector.connector_manifest)
    lineage = connector._create_lineage_for_table("public.users", parser)
    assert lineage is not None

    assert lineage.source_dataset == "mydb.public.users", (
        f"with a dotted database.dbname, only the first segment should be "
        f"used as the database; got {lineage.source_dataset!r}"
    )
