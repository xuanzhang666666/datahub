...
def test_parse_runtime_context_without_runner_uses_inline_context() -> None:
    ctx = parse_runtime_context(
        job_display_name="app_inline_job",
        shell_command="echo no runner",
        etl_content="select * from data_smartorder.ods_source",
        gitlab_file_path="datahub_structured_property:data_smartorder.app_inline_job:Execute Shell",
    )

    assert ctx.gitlab_name == ""
    assert ctx.project_path == ""
    assert ctx.job_path == "app_inline_job"
    assert ctx.job_type == "inline"
    assert ctx.job_file_name == "app_inline_job.sh"
    assert ctx.runtime_params == {}
