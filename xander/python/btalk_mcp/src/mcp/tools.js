...
const schemas = {
  ...
  ripple_action: z.object({
    ...
    show_form: z.boolean().default(false).describe('只查看表单字段，不提交'),
    auto: z.boolean().default(false).describe('使用默认值自动提交'),
    fields: z.record(z.string()).default({}).describe('表单字段，键值形式，如 {"alertReason":"888"}'),
  }),
...
const descriptors = [
  { name: 'list_conversations', description: '列出当前账号的所有会话(置顶+普通)', inputSchema: zodToJsonSchema(schemas.list_conversations) },
  { name: 'search_contact', description: '按 q 子串过滤会话列表', inputSchema: zodToJsonSchema(schemas.search_contact) },
  { name: 'fetch_history', description: '拉取会话历史消息', inputSchema: zodToJsonSchema(schemas.fetch_history) },
  { name: 'send_message', description: '发送文本消息', inputSchema: zodToJsonSchema(schemas.send_message) },
  { name: 'send_file', description: '上传并发送文件/图片(走蜂盘,100MB 上限)', inputSchema: zodToJsonSchema(schemas.send_file) },
  { name: 'get_image_path', description: '取图片/文件消息的本地缓存路径(用于 OCR/进一步处理)', inputSchema: zodToJsonSchema(schemas.get_image_path) },
  { name: 'btalk_status', description: '查看 btalk daemon 登录与运行状态。排查 MCP/btalk 是否可用时先调用。', inputSchema: zodToJsonSchema(schemas.btalk_status) },
  { name: 'wsso_cookie', description: '获取或强制刷新公司内部系统 SSO Cookie。Ripple 工单命令报响应解析失败/401 时用 reset=true。', inputSchema: zodToJsonSchema(schemas.wsso_cookie) },
  { name: 'ripple_list', description: '查询 Ripple/蜂利器流程工单列表，默认查待处理。可按 tab/category/keywords 过滤。', inputSchema: zodToJsonSchema(schemas.ripple_list) },
  { name: 'ripple_show', description: '查看 Ripple 工单详情，包括表单数据、处理历史、当前节点和可用操作。', inputSchema: zodToJsonSchema(schemas.ripple_show) },
  { name: 'ripple_action', description: '执行 Ripple 工单操作：领取 ACCEPT、处理 APPROVE、反馈 FEEDBACK、转交 ASSIGN。可 show_form 查看字段，或 fields 填表提交。', inputSchema: zodToJsonSchema(schemas.ripple_action) },
  { name: 'fetch_internal', description: '用 btalk wsso cookie 访问内部 HTTP 接口，适合调用公司内网页面/API。', inputSchema: zodToJsonSchema(schemas.fetch_internal) },
];
