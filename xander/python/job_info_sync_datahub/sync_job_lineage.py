...
根据传入的 job_display_name，自动完成：
  ...
  3. 调用 LLM（DeepSeek）提取目标表 / 上游表级血缘
...
必要依赖：
  pip install trino acryl-datahub openpyxl
"""
...
def run(
    ...
    try:
        ...
        gitlab_name = ""
        project_path = ""
        job_path = job_display_name
        kind = "inline"
        jfn = f"{job_display_name}.sh"
        candidates: list[str] = []
        if not is_inline:
        ...
        if etl_file:
            ...
            if not looks_like_inline_etl(metadata.shell_command):
                fallback = load_existing_structured_etl_source(
                    gms_url or "http://localhost:8080",
                    gms_token,
                    job_display_name,
    ...
    table_lineages: list = []
    field_lineages: list = []
    lineage_decision: Optional[LineageWriteDecision] = None
    try:
        ...
        if output_dir:
            lout = Path(output_dir) / job_display_name / "llm_extract.json"
            lout.parent.mkdir(parents=True, exist_ok=True)
            lout.write_text(json.dumps(llm_raw, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception as exc:
        logger.warning("LLM 提取失败，跳过血缘写入: %s", exc)
        lineage_decision = LineageWriteDecision(
        ...
        try:
            ...
            append_lineage_audit_jsonl(
                audit_path, job_display_name, lineage_decision,
    ...
    if not table_lineages:
        logger.warning(
            "ABNORMAL_JOB\tSKIP_NO_TABLES\t%s\tLLM 未提取到目标表",
            job_display_name,
        )
        return 3
    ...
    if lineage_decision is not None and not lineage_decision.write_upstream_lineage:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--llm-compare",
        action="store_true",
        help="已废弃；LLM 提取现为默认行为，保留此参数仅为向后兼容",
    )
    p.add_argument(
        "--llm-timeout",
        type=int,
    ...
    p.add_argument(
        "--lineage-vote",
        action="store_true",
        help="已废弃；不再执行多解析器投票，保留此参数仅为向后兼容",
    )
    p.add_argument(
        "--discrepancy-log",
        default=None,
